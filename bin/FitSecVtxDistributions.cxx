...
RooWorkspace *defineWorkspace(std::vector<SecVtxShape_t> &chShapes)
{
  ...
  // RooRealVar lxy("lxy","L_{xy} [cm]",0,10) ;
  ...
  for(size_t is=0; is<chShapes.size(); is++)
      ...
      for(std::map<Float_t,TH1F *>::iterator it=chShapes[is].lxy_signal.begin(); it!=chShapes[is].lxy_signal.end(); it++)
	{
	  ...
	  for(map<TString, TH1F *>::iterator bit=chShapes[is].lxy_bckg.begin(); bit != chShapes[is].lxy_bckg.end(); bit++) {
	    ...
	    std::cout << "adding background histogram " << bit->first << std::endl;
	  }
      ...
      // grmean->SetMaximum(0.95);
      // grmean->SetMaximum(0.55);


      grmean->Draw("p");  grmean->Fit("pol1");grmean->SetName(ch+"meanevol");

      std::cout << "Fit parameters (mean): " << grmean->GetFunction("pol1")->GetParameter(0) << " / " << grmean->GetFunction("pol1")->GetParameter(1) << std::endl;
      ...
      std::cout << "Fit parameters (q25): " << gr25->GetFunction("pol1")->GetParameter(0) << " / " << gr25->GetFunction("pol1")->GetParameter(1) << std::endl;
      ...
      std::cout << "Fit parameters (q50): " << gr50->GetFunction("pol1")->GetParameter(0) << " / " << gr50->GetFunction("pol1")->GetParameter(1) << std::endl;
...
int main(int argc, char *argv[])
{
  ...
  while(chDir!=0)
      ...
      while(nextTemplKey!=0)
	{
	  ...
	  if((var.Contains("Nm1") || var.Contains("eta") || var.Contains("pt") || var.Contains("presel") || var.Contains("Count")|| var.Contains("PV") || var.Contains("phi") || var.Contains("njets") || var.Contains("1jet") )
	  ...
	  if(proc=="data")
		...
		{
		  std::cout << "Adding to the background: " << ch << " " << var << " " << proc << std::endl;
