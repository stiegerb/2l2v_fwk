...
class toyResult():
    ...
    def doCalibrationStep(self,workspace,nToys,nSamples,fittype,floatSignal,outdir):
        ...
        if self.mass not in masses:
            ...
            self.mass = '1725'
            if syst.find('dy')<0  :
                print self.channel+self.variable+'_'+syst+'syst'
        ...
        self.toycorrHisto = ROOT.TH2F('toycorr_histo_'+self.channel+'_'+self.mass, ';m_{top}(mean) vs. m_{top}(quantile);entries/bin',  1000,150,190,1000,150,190)
        ...
        for toy in xrange(nToys):
            ...
            self.toycorrHisto.Fill(mtop_mean,mtop_50q)
    ...
    def makeControlPlots(self,method,resultsSummary,outdir):
        ...
        biasH    = ROOT.TH1F(method+'bias_histo_'+self.channel+'_'+self.mass, ';Fitted m_{top}-Generated m_{top} [GeV];Toys / 0.5 GeV',  100,  -10., 12.5)
...
channels = ['mu']
systematics = ['mepsdown','mepsup','q2down','q2up','powheg','jesup','jesdown']
# systematics = ['jesup','jesdown','powheg','mcatnlo']
# systematics = ['xbup','xbdown']
...
|_| |_| |_|\__,_|___/___/_| |_|\__|\__\___|_|   
    ...
    if runMode == 'syst':
        ...
        for systematic in systematics:
            ...
            print '+++ systematics sample: ',systematic
    ...
    if runMode == 'all' or runMode == 'toys':
        ...
        for tag in allFitResults:
            ...
            allFitResults[tag].toycorrHisto.Write()
        biasoutfile.Close()
    ...
    if runMode == 'all' or runMode == 'calib':
        ...
        ## plot the mean median shift
        gr_toydiff = ROOT.TGraphAsymmErrors()
        i=0
        for mass in masses:
            ROOT.gStyle.SetOptFit(1)
            ROOT.gStyle.SetOptStat(0)
            ...
            c_hist_diff = TCanvas('c_hist_diff'+'_'+chan+'_'+mass,'c_hist_fit_'+'_'+chan+'_'+mass,400,400)
            hist = getHist(rfile, 'toydiff_histo_'+chan+'_'+mass)
            hist.Sumw2()
            hist.SetTitle('m_{gen} = '+str(float(mass)/10.)+' GeV, '+chan+' channel')
            hist.SetLineColor(1)
            hist.SetMarkerStyle(20)
            ...
            gf = TF1('f1','gaus',-20.,20)
            gf.SetLineWidth(2)
            gf.SetLineStyle(1)
            gf.SetLineColor(8)
            gf.SetParameters(50.,0.,5.)
            ## p0: height, p1: mean, p2: width, 
            hist.Fit('f1','Q')
            mean = gf.GetParameter(1)
            width = gf.GetParameter(2)
            gf.Draw("same")
            hist.Draw('e1 same')
            ...
            if mass=='1615':
                massRanHi=1
            if mass=='1615' or mass=='1635':
                massRanLo=1
            np=h_bias.GetN()
            gr_toydiff.SetPoint(i,float(mass)/10.,mean)
            gr_toydiff.SetPointError(i,massRanLo,massRanHi,width,width)
            i+=1

            cms = cmsPreliminary()
            cms.Draw()
            
            leg = ROOT.TLegend(0.1,0.85,0.45,0.85)
            leg.SetFillColor(0)
            leg.SetLineColor(0)
            ...
            c_hist_diff.Modified()
            c_hist_diff.Update()
            c_hist_diff.Print(opt.outputdir+'/c_toydiff_'+chan+'_'+mass+'.pdf')
            c_hist_diff.Print(opt.outputdir+'/c_toydiff_'+chan+'_'+mass+'.png')

        c_mean_median_diff = TCanvas('c_mean_median_diff'+'_'+chan,'c_mean_median_fit_'+'_'+chan,400,400)
        c_mean_median_diff.cd()
        #fit a straight line to the calibration bias and its stat envelope
        lfunc = TF1('lf','[0]+[1]*x',150,190)
        lfunc.SetLineWidth(2)
        lfunc.SetLineColor(4)
        lfunc.SetParameters(0.,0.)
        gr_toydiff.GetYaxis().SetTitle('#Delta m_{top} (mean-quantile)')
        gr_toydiff.Fit(lfunc,'Q')
        a = lfunc.GetParameter(0)
        b = lfunc.GetParameter(1)
        gr_toydiff.GetYaxis().SetRangeUser(-10,10)
        gr_toydiff.GetXaxis().SetTitle('m_{gen} [GeV]')
        gr_toydiff.GetYaxis().SetTitle('#Delta m_{top} (mean-median) [GeV]')
        gr_toydiff.SetMarkerStyle(20)
        gr_toydiff.SetMaximum(2.)
        gr_toydiff.SetMinimum(-2.)
        gr_toydiff.Draw('ap')
        lfunc.Draw('lsame')
        cms = cmsPreliminary()
        cms.Draw()

        c_mean_median_diff.Modified()
        c_mean_median_diff.Update()
        c_mean_median_diff.Print(opt.outputdir+'/c_toydiff_'+chan+'.pdf')
        c_mean_median_diff.Print(opt.outputdir+'/c_toydiff_'+chan+'.png')
    ...
    if runMode == 'all' or runMode == 'fit':
        ...
        for chan in channels:
            ...
            for method in methods:
                ...
                #fit a straight line to the calibration bias and its stat envelope
                lfunc = TF1('lf_'+method,'[0]+[1]*x',150,190)
                ...
                if method=='fit' :
                    ...
                    #Juerg: are you sure this gives the correct error?
                    ...
                    sigfrac = workspace.var(chan+'sigfrac')
                    sigfrac.setConstant(kTRUE)
                    mod_sig  = workspace.pdf(chan+'lxy_1725')                    
                    mod_bkg  = workspace.pdf(chan+'lxy_bckg')
                    mod_gen = RooAddPdf('s+b_model_'+chan+'_1725','s+b_model_'+chan+'_1725',RooArgList(mod_sig, mod_bkg),RooArgList(sigfrac))
                    observable=workspace.var('lxy')
                    binnedMC=mod_gen.createHistogram("tmp",observable)
                    xq=array('d',[0.25,0.5,0.75,1])
                    yq=array('d',[0,   0,  0,   0])
                    binnedMC.GetQuantiles(4,yq,xq)
            
                    invFormula=workspace.function(chan+'meanInv')
                    obsVal=binnedMC.GetMean()
                    observable.setVal(obsVal)
                    mtop_mc_raw    = invFormula.getVal()
                    mtop_mc_calib = (mtop_mc_raw-pa.getVal())/(1 + pb.getVal())
                    mtop_mc_diff = mtop_mc_calib - 172.5

                    fOut.write( ' Calibration uncertainty @ 172.5 GeV: %f \n'% mtop_mc_diff)
                    binnedMC.Delete()
